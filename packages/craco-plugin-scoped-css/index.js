...
module.exports = {
  overrideWebpackConfig: ({
    ...
    if (isFound) {
      match.loader.options.plugins.unshift(require.resolve('babel-plugin-react-scoped-css'))
    } else {
    ...
    if (!lessRule) {
      console.log(
        "Can't find the webpack rule to match less files in the " + `${env} webpack config!`,
      )
    } else {
